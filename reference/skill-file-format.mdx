...
| `description` | string | Universal | Recommended | One-paragraph description of what the skill does and when to use it. Shown in Rundock's interface and read by the agent once it opens the file. Rundock does not send it to the model: the slug is what the agent is given. |
...
<Note>
**Only agents that are on the team are considered.** An agent whose status is `available` or `raw` is skipped entirely and gets no skills however it declares them. Nothing warns, so this reads as the assignment being broken.

The `rundock-` prefix then gates in both directions, as two separate rules:

- A **platform** agent, currently only Doc, is offered `rundock-` skills and nothing else.
- Every **other** agent is offered everything except `rundock-` skills.

Neither case warns. The skill simply does not attach.
</Note>
...
**Both `skills:` forms parse.** The flow style `skills: [a, b]` is tried first and the block form is the fallback. An earlier version of this page said the flow form parsed as an empty array; it does not, and never did.
