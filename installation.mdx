...
Open a fresh PowerShell window and run `claude --version`. If the command fails, Claude Code is not on PATH for your user. Install it (or reinstall it) via Anthropic's PowerShell installer:

```powershell
irm https://claude.ai/install.ps1 | iex
```

Fully close Rundock and reopen it from the Desktop shortcut so the launcher picks up the updated PATH. Rundock's Windows detection handles both Anthropic's PowerShell installer (`claude.exe`) and the npm install (`claude.cmd`); either path works.
