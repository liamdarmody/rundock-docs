...
The index is a small file inside your workspace's `.rundock/` folder, rebuilt automatically. Rundock sends it nowhere. It does sit inside the workspace folder, so on a workspace shared through a sync tool it travels with everything else in that folder unless `.rundock/` is excluded: see [Workspaces](/concepts/workspaces). Where the index engine is not available, search falls back to a simpler scan rather than failing.
