...
- **Skills are visible by name.** An agent is told which skills it owns as a list of slugs, so a capability written into a skill file can be reached by name, while one buried in an agent's body cannot.
...
<Frame caption="The skills panel shows what each skill is and which agents own it. The Used By row makes assignment visible without opening any agent file.">
  <img src="/images/skills.png" alt="Rundock skills view showing the list of workspace skills with one skill open, its description, and the agents it is attached to." />
</Frame>
...
What reaches the model is the skill's **slug**, and nothing else. Rundock injects a bare list (`Skills: alpha, beta`) into the agent's prompt: no description, no summary. The agent opens the file from that name and reads the body to decide what to do.

This means **the slug is doing the work the description looks like it is doing**, so name the skill for what it is for: `linkedin-hook-generator` rather than `content-helper`. The description is for people, in Rundock's interface and in review, and it is still worth writing well.
