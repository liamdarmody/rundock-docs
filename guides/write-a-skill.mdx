...
- **`description`** is one paragraph, written for people. It appears in Rundock's interface and is read by the agent once it opens the file, but **Rundock does not send it to the model**, so it is not what gets a skill chosen. The **slug** is. Name the skill for what it is for, and write the description as though someone has to choose between this skill and its neighbour.
...
Either form works. Rundock tries the flow style (`skills: [weekly-report]`) first and falls back to the block form (`-` lines indented under `skills:`), stripping quotes either way. An earlier version of this page said the flow form parsed as empty; it does not.
...
2. **Is the slug specific enough?** The slug is the only part the model sees when deciding whether to open the skill. A vague name is the thing that gets a skill overlooked, not a vague description.
